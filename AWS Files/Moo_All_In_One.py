...
###objects to go in setupfile
    ...
    def ThreeDepth():
        #####Load Specific filename and create an array of x,y,temp,rho and salt
        filename = '40km_Drho46_2D'
        ice_filename = filename
        xcord,ycord,temp,rho,saltconc = Fileread2D(filename)
        ...
        #Returns surface if it can communicate back to surface
            ...
            for i in comm_range_list:
                ...
                x_current = Probe_position
                x_current += 100
                Sci_calc =CommAbove4(temp_profile_list[profile_index],Probe_position,x_surface,Frequency,c_ice,salt_profile_list[profile_index],P_tx_Sci,eff_tx_Sci,G_tx_Sci,eff_rx_Sci,G_rx_Sci,k_boltz,T_sys_Sci,BW,Target_SNR)
                Rel_calc = CommAbove4(temp_profile_list[profile_index],Probe_position,x_surface,Frequency,c_ice,salt_profile_list[profile_index],P_tx_Rel,eff_tx_Rel,G_tx_Rel,eff_rx_Rel,G_rx_Rel,k_boltz,T_sys_Rel,BW,Target_SNR)
                Sci_Max_Comm_List.append(Sci_calc)
                Rel1_Max_Comm_List.append(Rel_calc)
                #print(g)       
                #Probe_position +=1
            #print(Max_Comm_List)
            #print(Maxx_Comm_List[-1])
            Sci_Max_Comm_spline = CubicSpline(comm_data_list,Sci_Max_Comm_List)
            Rel1_Max_Comm_spline = CubicSpline(comm_data_list,Rel_Max_Comm_List)

            Max_Comm_Height_list_Sci.append(Sci_Max_Comm_spline)
            Max_Comm_Height_list_Rel.append(Rel1_Max_Comm_spline)
        ...
        for profile_index in range(len(little_indexes)):
            ...
            V_0_Science = VelocityCurrent4(Science_Probe_Profile,Temp_surface,Rho_surface,Tm_surface)
            V_0_Relay1 = VelocityCurrent4(Relay_Probe_Profile,Temp_surface,Rho_surface,Tm_surface)
            
            V_0_Science_mh = V_0_Science*3600
            V_0_Science_kmd = V_0_Science*86.4

            V_0_Relay1_kmd = V_0_Relay1*86.4

        ####For Science&Relay: Go through each depth and caculate the velocity of the probes

        Science_Velocity_profiles = []
        Relay1_Velocity_profiles = []
        Temp_profiles = []
        Melt_point_profiles = []
        for profile_index in range(len(little_indexes)):
            Science_Velocity_List = []
            Relay1_Velocity_List = []
            TempList =[]
            for x_position in x_range_list[profile_index]:
                Tm_current= Mp_w_ScenarioSpline_list[profile_index](x_position)
                Temp_current = temp_profile_list[profile_index](x_position/1000)
                ...
                if Sci_CRF == True:
                    ...
                    Science_Velocity_List.append(Science_V_Current*86.4)
                #print(Science_V_Current)
                Relay1_V_Current = VelocityCurrent4(Relay_Probe_Profile,Temp_current,Rho_current,Tm_current)
                Relay1_Velocity_List.append(Relay1_V_Current*86.4)
            
            #print(Science_Velocity_List)
            Science_Vel_splined = CubicSpline(x_range_list[profile_index],Science_Velocity_List)
            Relay1_Vel_splined = CubicSpline(x_range_list[profile_index],Relay1_Velocity_List)
            Temp_splined = CubicSpline(x_range_list[profile_index],TempList)
            
            
            Science_Velocity_profiles.append(Science_Vel_splined)
            Relay1_Velocity_profiles.append(Relay1_Vel_splined)
            Temp_profiles.append(Temp_splined)
        ...
        Science_Total_Power_list = []
        Science_Energy_per_m_list = []
        Relay1_Total_Power_list = []
        Relay1_Energy_per_m_list = []

        Break_Reasons = []

        for profile_index in range(len(little_indexes)):
            Science_v = Science_Velocity_profiles[profile_index]
            Relay1_v = Relay1_Velocity_profiles[profile_index]
            x_0 = max(x_range_list[profile_index])
            
            Science_Transit_list = [0]
            Relay1_Transit_list =[0]
            Sci_Max_Comm_check = Max_Comm_Height_list_Sci[profile_index]
            Relay1_Max_Comm_check = Max_Comm_Height_list_Rel[profile_index]
            ...
            for t in timeline:
                ...
                Relay1_v_current = Relay1_v(Relay1_x)*(1000 / (24 * 3600))
                Relay1_x -= Relay1_v_current*timestep_s
                Relay1_Transit_list.append(Relay1_x-x_0)
                ...
                if int(Science_x) < int(min(x_range_list[profile_index])):
                    ...
                    Science_Total_Power_list.append(Qdot_Science*t)
                    Science_Energy_per_m_list.append(-Qdot_Science*t/(min(x_range_list[profile_index])-x_0))
                    Relay1_Total_Power_list.append(Qdot_Relay*t)
                    Relay1_Energy_per_m_list.append(-Qdot_Relay*t/(min(x_range_list[profile_index])-x_0))
                ...
                elif int(Relay1_Max_Comm_check(Relay1_x)) < int(x_0):
                    Transit_times_s.append(t)
                    Max_Depth_Reached.append(Science_x-x_0)
                    ...
                    Break_Reasons.append('Relay 1 Comms')
                    #print('c')
                    break

                elif int(Sci_Max_Comm_check(Science_x)) < int(Relay1_x):
                    Transit_times_s.append(t)
                    Max_Depth_Reached.append(Science_x-x_0)
                    Science_Total_Power_list.append(Qdot_Science*Mission_Time_Limit_s)
                    Science_Energy_per_m_list.append(-Qdot_Science*Mission_Time_Limit_s/(min(x_range_list[profile_index])-x_0))
        ...
        #Transit_times_s = [element*0.93 for element in Transit_times_s]
        Transit_time_mean = np.mean(Transit_times_s)
        Transit_time_sd = np.std(Transit_times_s)
        Transit_time_min = (min(Transit_times_s))
        Transit_time_max = (max(Transit_times_s))
        ...
        #Max_Depth_mean = [element*0.93 for element in Max_Depth_Reached]
        Max_Depth_mean = np.mean(Max_Depth_Reached)
        Max_Depth_sd = np.std(Max_Depth_Reached)
        Max_Depth_min = (min(Max_Depth_Reached))
        Max_Depth_max = (max(Max_Depth_Reached))
        ...
        Max_data = np.multiply(Frequency,Transit_times_s)

        return(Max_Depth_mean,Transit_times_s)
...
class MyProblem(ElementwiseProblem):
    ...
    def _evaluate(self, vars, out, *args, **kwargs):
        ...
        f2 = -result[1]*BW*np.log2(1+Target_SNR)
        #f2 = -result[1]
